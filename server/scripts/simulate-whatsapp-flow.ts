async function main() {
  const [
    { initializeDatabase },
    { processInboundMessage, rememberLastOutboundAssignment },
    taskService,
    { config }
  ] = await Promise.all([
    import("../src/db/init.js"),
    import("../src/services/message-service.js"),
    import("../src/services/task-service-async.js"),
    import("../src/config.js")
  ]);

  await initializeDatabase();

  const assignmentsBefore = await taskService.listAssignmentsAsync();
  ...
  const cases = [
    { label: "HELP", from: "whatsapp:+4917613420040", body: "HELP" },
    { label: "TASKS", from: "whatsapp:+4917613420040", body: "TASKS" },
    { label: "STATUS", from: "whatsapp:+4917613420040", body: "STATUS" },
    {
      label: "NATURAL_LANGUAGE_SKIP_REASSIGN",
      from: "whatsapp:+4917613420040",
      body: "I can't do it today, skip"
    },
    { label: "DONE", from: "whatsapp:+4917613420040", body: `DONE ${varunAssignment.id}` },
    {
      label: "SKIP",
      from: "whatsapp:+491700000101",
      body: `SKIP ${mayssaAssignment.id} running late tonight`
    },
    { label: "RESCUE", from: "whatsapp:+491700000104", body: `RESCUE ${noahAssignment.id}` },
    { label: "UNKNOWN", from: "whatsapp:+4917613420040", body: "WHO OWES WHAT RIGHT NOW?" }
  ];
  ...
  const finalAssignments = (await taskService.listAssignmentsAsync()).map((assignment) => ({
    id: assignment.id,
  ...
  const penalties = (await taskService.listPenaltiesAsync()).map((penalty) => ({
    id: penalty.id,
